...
In the final dataset we have 43089 observations of 13 variables (Strain, Isolate.identifiers and Isolation.type, SNP.cluster, Min.same and Min.diff, AMR.genotypes, Year, Month, Source, State, and Outbreak). The following variables does not have missing values:Isolate, Year, Month, and Outbreak. The biggest number of missing values is observed in State variable (15362 observations). The following variables - Isolate.identifiers, Strain, and Source have respectively small number of missing observations (14, 39, and 222 respectively).
The dataset consists of 36% clinical isolates and 64% environmental isolates (the number of missing values is small - 51 observation). In terms of environmental source the highest number of isolates was collected from chicken (17296), followed by cattle (6739) and pig (1602); the smallest number - from dairy (88 isolates). It is important to mention that the dataset does not contain isolates from water sources that also is a high-risk source of infection.
...
The distribution of cases by month varies from 6.5% in February to almost 12% in April. The most number of isolates were collected in spring and summer (11.5 thousands per each season), and slightly less in fall (10.7 thousands), and winter (9.2 thousands).
...
1844 isolates were assigned to outbreaks, representing 607 outbreaks in total.
...
From the bar graph above, we can observe that most of the sources came from chicken, having the highest percentage, followed by human, cattle and pig as the top 4 most sources. Dairy provided the least source as per the graph.
...
From the graph, we can observe that the year 2019 was the most prevalent year with the highest performance in terms of isolates data. However, 2012 had the lowest frequency. Notably, as the years progress, their frequencies keep increasing, meaning that there are improvements in the upcoming years. 
...
Notably, from the graph, we can observe that environmental/other had the highest isolation type, with 2019 having the highest environmental/other isolation type. Also, from the graph, we can observe that 2012 had the lowest isolation types for both clinical and environmental. As the years progressed, the isolation type for each section increased immensely. This graph enables us to predict that in the future, there is still the possibility of having increased isolation.

Clinical isolates are less popular than environmental since they may not prevent diseases from occurring due to the sources in use. Environmental isolation types are more preferred since there are diverse sources from different environments. Also, environmental/other isolation types could be more preferred than clinical due to their cost efficiency. That is, they are less costly.
