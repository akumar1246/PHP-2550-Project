...
# Make sure our isolate variables for Year and Month are in numeric format
...
isolates$Outbreak <- ifelse(isolates$match %in% outbreaks, 1, 0)
...
As we see in Figure 1, we do not have many variables with a high percentage of missing values after whittling down our dataset to variables that may be useful, and cleaning variables that we would like to use. The two variables with the highest percentages are *Min.diff* and *State*, which have 36% and 35% of their values missing, respectively. Other variables with missing values include *Source* which has approximately 23% of its values missing, *Min.same* which has about 18% of its values missing, *SNP.cluster* which has 16% of values missing, and *AMR.genotypes* which has 8% of values missing. The remaining variables with missing values have a negligible (less than 0.1%) percentage of missing values.
...
As we see in Figure 2, observations that are missing *Min.diff* also tend to be missing *Min.same* and *SNP.cluster*. This makes sense, as these variables are all related to SNPs and will all be missing if samples do not undergo SNP genotyping. Furthermore, we see that observations that are missing *State* slightly tend to also be missing *Source*, which could partially be due to those who are entering the isolates in the database only filling out the required fields, but we can't say for certain. The large majority of missing values are likely for reasons that are unknown to us, therefore our data is missing not at random (MNAR). This is a large limitation to our analysis, as we may estimate biased results. In the future, the NCBI Pathogen Detection Project should consider reforming their data collection process, for example changing the submission to require most or all fields, if they would like to use the data to estimate accurate trends or predictions.
...
For the 3,220 isolates that are potentially linked to outbreaks, we have 3,215 unique categories for Strain. For the 39,869 isolates that are not linked to outbreaks, we have 39,789 unique categories for Strain. The majority of isolates seem to have unique strains for both outbreak-linked and non-outbreak-linked isolates, therefore, I do not see a strong relationship between these two variables.
...
For the 3,220 isolates that are potentially linked to outbreaks, we have 3,218 unique categories for isolate identifiers. For the 39,869 isolates that are not linked to outbreaks, we have 39,851 unique categories for isolate identifiers. The majority of isolates seem to have unique isolate identifiers for both outbreak-linked and non-outbreak-linked isolates, therefore, I do not see a strong relationship between these two variables.
...
For the 3,220 isolates that are potentially linked to outbreaks, we have 933 unique categories for SNP cluster. For the 39,869 isolates that are not linked to outbreaks, we have 3,809 unique categories for SNP cluster. In other words, 29.0% of isolates linked to outbreaks have a unique SNP cluster while 9.6% of isolates not linked to outbreaks have a unique cluster. This tells us that isolates that are linked to outbreaks may be more diverse; and that outbreaks may not be associated with a specific group of SNP clusters. Instead, it could be possible that there are large groups of SNP clusters that are unlikely to result in an outbreak. As a result, we conclude that this variable seems to have a relationship with outbreak.
...
For the 3,220 isolates that are potentially linked to outbreaks, we have 392 unique categories for AMR genotypes For the 39,869 isolates that are not linked to outbreaks, we have 1,734 unique categories for AMR genotypes. In other words, 12.2% of isolates linked to outbreaks have a unique AMR genotype while 4.3% of isolates not linked to outbreaks have a unique AMR genotype. Again, this tells us that isolates linked to outbreaks may be more genetically diverse, or that there are large groups of isolates that are genetically similar and are unlikely to result in an outbreak. Another possibility is that less commonly seen strains / more mutated strains are more likely to cause an outbreak because they are treatment resistant.
